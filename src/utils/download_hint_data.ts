...
import { resolveURL, compressHTML, getText, loadHtml, print, initHttpCache, shouldBeEqual, AssertLevel, bold, lengthShouldBeMoreThanOrEqual, lengthShouldBeEqual, toMarkdown } from "./helper";

const manifestStreams = {
	core: createWriteStream(manifestFiles.core),
	js: createWriteStream(manifestFiles.js),
}
...
async function main() {
	...
	manifestStreams.core.write('[\n');
	manifestStreams.js.write('[\n');

	manifestStreams.core.write(JSON.stringify(coreModuleNames));
	manifestStreams.js.write(JSON.stringify(jsModuleNames));
...
async function processModuleDocs(context: {
	moduleName: string
	moduleIndex: number
	uri: string
}) {
	...
	const detailsStream = createWriteStream(detailsFile(moduleName));
	detailsStream.write('[');
	...
	$directives.each(i => {
		...
		const contexts = directiveContext.text().split(',')
			.map(it => {
				it = it.trim()
		...
		while ((elementPointer =
			...
			switch (tagName) {
				...
				case 'BLOCKQUOTE': {
					...
					if (className == 'note')
						markdownNotes.push(toMarkdown(elementPointer.html()));
		...
		if (syntaxArray.length === 1) {
			...
			if (testParams === 'on|off') {
				...
				if (mtx)
					ci.insert = `${directiveName} $\{1:${mtx[1]}\} $\{2:${mtx[2]}\};$0`
				else if (placeholder)
					ci.insert = `${directiveName} $\{1:${placeholder}\};$0`;
				else
					ci.insert = `${directiveName};$0`;
			}
		}
		const ciArgs: string[] = [];
		...
		detailsStream.write(count.details++ > 0 ? ',\n' : '\n');
		detailsStream.write(JSON.stringify([
	...
	if ($varContainer.length) {
		...
		shouldBeEqual(`variables info of ${moduleName}: a[name=variables]+center+p+dl or ` +
		...
		container.children('dt').each((i, e) => {
			...
			lengthShouldBeMoreThanOrEqual(`attribute "id" of element "dt" ${varName} `,
		...
		detailsStream.write(count.details++ > 0 ? ',\n' : '\n');
		detailsStream.write(JSON.stringify([
			ManifestItemType.VariableDetails,
			fullDocs,
		]));
	}
